...
namespace nador
{
	Image::Image(const uint32_t width, const uint32_t height, const std::string& name, const std::string& atlasName, const video::EImageName id, const glm::mat4x2& uvs)
	: width(width)
	, height(height)
	, name(name)
	, atlasName(atlasName)
