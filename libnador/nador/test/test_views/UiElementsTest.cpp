...
namespace nador
{
    ...
    UiElementsTest::UiElementsTest(IUiApp* uiApp, const IFontController* fontCtrl, const IInputController* inputCtrl)
    : _uiApp(uiApp)
    , _fontCtrl(fontCtrl)
    , _inputCtrl(inputCtrl)
    {
        ...
        FontPtr font = _fontCtrl->GetDefaultSystemFont();
        ...
        if (font)
        {
            UiEditText::UiEditTextImages editTextImages { video::EImageName::RECTANGLE,
                                                          video::EImageName::RECTANGLEDOWN,
                                                          video::EImageName::RECTANGLEOVER };
    ...
    void UiElementsTest::OnDebugRender()
    {
        ...
        auto currentMousePosition = _inputCtrl->GetMousePosition();

        if (_inputCtrl->IsMouseButtonReleased(EMouseButton::LEFT))
        {
        ...
        {
            _uiApp->DebugDrawEdge(!_uiApp->IsDebugDrawEdge());
        }
    ...
    void UiElementsTest::SelectElementAndAddToLayer(IUiElement* elem, std::string_view name, EUiLayer layer)
    {
        ...
        _uiApp->AddElementToLayer(layer, elem);
    }
